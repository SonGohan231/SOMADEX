...
static func _draw_kotwiczek(image: Image) -> void:
	...
	_set_block(image, 22, 21, MOSS, 1)
	_set_block(image, 34, 20, MOSS_D, 1)
	_ellipse(image, Rect2i(20, 29, 7, 9), CYAN)
	_ellipse(image, Rect2i(22, 30, 3, 6), OUTLINE)
	_set_block(image, 23, 31, WHITE, 0)
	_ellipse(image, Rect2i(32, 29, 7, 9), CYAN)
	_ellipse(image, Rect2i(34, 30, 3, 6), OUTLINE)
	_set_block(image, 35, 31, WHITE, 0)
	_line(image, Vector2i(27,39), Vector2i(30,41), OUTLINE, 0)
	_line(image, Vector2i(30,41), Vector2i(33,39), OUTLINE, 0)
	for leg: Array[Vector2i] in [
		...
		_line(image, leg[0], leg[1], RUST_D, 2)
		_line(image, leg[1], leg[2], RUST_L, 2)
	_line(image, Vector2i(42,39), Vector2i(53,43), ROCK_D, 2)
	_line(image, Vector2i(53,43), Vector2i(55,25), ROCK_D, 2)
	_line(image, Vector2i(55,25), Vector2i(59,21), ROCK_L, 1)
	_line(image, Vector2i(55,25), Vector2i(51,21), ROCK_L, 1)
	_line(image, Vector2i(55,43), Vector2i(49,50), ROCK_D, 2)
	_line(image, Vector2i(55,43), Vector2i(61,50), ROCK_D, 2)
	_line(image, Vector2i(49,50), Vector2i(46,47), ROCK_L, 1)
	_line(image, Vector2i(61,50), Vector2i(63,47), ROCK_L, 1)
	_set_block(image, 55, 36, ROCK_L, 1)

static func _draw_bramnik(image: Image) -> void:
	_ellipse_outline(image, Rect2i(7, 23, 43, 28), OUTLINE, ROCK)
	for r: Rect2i in [Rect2i(10,21,11,9),Rect2i(18,18,12,10),Rect2i(28,19,12,10),Rect2i(37,22,11,9)]:
		_ellipse_outline(image, r, ROCK_D, ROCK_L)
	_set_block(image, 15, 21, MOSS, 1)
	_set_block(image, 27, 18, MOSS_D, 1)
	_set_block(image, 40, 22, MOSS, 1)
	_poly(image, [Vector2i(36,29),Vector2i(49,24),Vector2i(58,29),Vector2i(56,38),Vector2i(47,41),Vector2i(37,37)], ROCK_L)
	_poly(image, [Vector2i(45,26),Vector2i(51,22),Vector2i(54,29)], WHITE)
	_poly(image, [Vector2i(38,29),Vector2i(43,24),Vector2i(46,31)], WHITE)
	_ellipse(image, Rect2i(48,29,5,5), CYAN)
	_ellipse(image, Rect2i(50,30,2,3), OUTLINE)
	_ellipse(image, Rect2i(55,33,5,4), OUTLINE)
	for leg: Array[Vector2i] in [
		...
		_line(image, leg[0], leg[1], ROCK_D, 3)
		_line(image, leg[1], leg[2], ROCK_L, 2)
	_line(image, Vector2i(43,39), Vector2i(48,48), ROCK_D, 2)
	_draw_partial_ring(image, Vector2i(52,49), 8, 205, 520, ROCK_D, 2)
	_draw_partial_ring(image, Vector2i(52,49), 6, 205, 520, ROCK_L, 1)
	_line(image, Vector2i(56,53), Vector2i(61,57), ROCK_D, 2)
	_line(image, Vector2i(61,57), Vector2i(63,53), ROCK_L, 1)
...
static func _draw_fundamentor(image: Image) -> void:
	...
	for x: int in [17,27,38,47]:
		_poly(image, [Vector2i(x-2,18),Vector2i(x-2,10),Vector2i(x,7),Vector2i(x+2,10),Vector2i(x+2,19)], ROCK_L)
		_set_block(image, x, 12, AMBER, 1)
	_poly(image, [Vector2i(24,27),Vector2i(29,22),Vector2i(37,22),Vector2i(42,27),Vector2i(38,36),Vector2i(28,36)], ROCK_D)
	_ellipse(image, Rect2i(27,26,6,6), AMBER)
	_ellipse(image, Rect2i(35,26,6,6), AMBER)
	_set_block(image, 30, 28, OUTLINE, 0)
	_set_block(image, 38, 28, OUTLINE, 0)
	_line(image, Vector2i(16,22), Vector2i(24,20), MOSS, 1)
	_line(image, Vector2i(41,20), Vector2i(49,23), MOSS_D, 1)
	...
	_line(image, Vector2i(17,32), Vector2i(7,39), ROCK_D, 3)
	_line(image, Vector2i(7,39), Vector2i(3,49), RUST_D, 3)
	_line(image, Vector2i(3,49), Vector2i(8,55), RUST_L, 2)
	_line(image, Vector2i(3,49), Vector2i(0,55), RUST_L, 2)
	_line(image, Vector2i(50,31), Vector2i(58,38), ROCK_D, 3)
	_line(image, Vector2i(58,38), Vector2i(59,51), ROCK_D, 3)
	_draw_partial_ring(image, Vector2i(55,53), 8, 110, 360, ROCK_D, 2)
	_draw_partial_ring(image, Vector2i(55,53), 6, 110, 360, ROCK_L, 1)
	_line(image, Vector2i(56,58), Vector2i(62,61), ROCK_D, 2)

static func _make_frame(base: Image, action: String, frame: int) -> Image:
	match action:
		"idle":
			var ys: Array[int] = [0,-1,0,1]
			return _place(base, 0, ys[frame], 1.0, 1.0, 1.0)
		"attack":
			var dx: Array[int] = [0,4,10,18,9,1]
			var dy: Array[int] = [0,0,-2,-4,-1,0]
			var result: Image = _place(base, dx[frame], dy[frame], 1.04 if frame in [2,3] else 1.0, 0.98 if frame == 3 else 1.0, 1.0)
			if frame in [2,3,4]:
				_draw_anchor_arc(result, 88 + int(dx[frame] / 2), 61, frame)
			return result
		"hurt":
			var hx: Array[int] = [-5,5,0]
			var hurt: Image = _place(base, hx[frame], 2 if frame < 2 else 0, 0.98, 0.98, 1.0)
			_tint_red(hurt, 0.46 if frame < 2 else 0.16)
			return hurt
		"faint":
		...
		"special":
			...
			var pulse: float = 1.0 + 0.05 * sin(p * PI)
			var special: Image = _place(base, 0, -int(round(2.0 * sin(p * PI))), pulse, pulse, 1.0)
			_draw_stability_ring(special, 24 + int(round(11.0 * sin(p * PI))), Color(0.93,0.67,0.35,0.70))
			_draw_stability_ring(special, 36 + int(round(10.0 * p)), Color(0.35,0.84,0.88,0.50))
			_draw_anchor_glyph(special, Vector2i(64,64), 10 + frame)
			_draw_dust(special, frame)
			return special
...
static func _draw_anchor_arc(image: Image, center_x: int, center_y: int, phase: int) -> void:
	...
	for degree: int in range(-120, 41, 8):
		...
		var y: int = center_y + int(round(sin(angle) * float(radius)))
		_set_block(image, x, y, Color(0.72,0.76,0.82,0.74), 1)
	for i: int in range(4):
		_set_block(image, center_x + radius - i * 2, center_y - 1 + i, CYAN, 1)

static func _draw_stability_ring(image: Image, radius: int, color: Color) -> void:
	for degree: int in range(0,360,12):
		var angle: float = deg_to_rad(float(degree))
		_set_block(image,64 + int(round(cos(angle) * float(radius))),64 + int(round(sin(angle) * float(radius))),color,1)

static func _draw_anchor_glyph(image: Image, center: Vector2i, size: int) -> void:
	var c: Color = Color(1.0,0.78,0.35,0.80)
	_line(image, Vector2i(center.x,center.y-size), Vector2i(center.x,center.y+size), c, 1)
	_draw_partial_ring(image, Vector2i(center.x,center.y-size+1), maxi(2,int(size/4)), 0, 360, c, 1)
	_line(image, Vector2i(center.x-size,center.y+int(size*0.4)), Vector2i(center.x+size,center.y+int(size*0.4)), c, 1)
	_line(image, Vector2i(center.x-size,center.y+int(size*0.4)), Vector2i(center.x-int(size*0.55),center.y+size), c, 1)
	_line(image, Vector2i(center.x+size,center.y+int(size*0.4)), Vector2i(center.x+int(size*0.55),center.y+size), c, 1)
...
static func _draw_dust(image: Image, phase: int) -> void:
	...
	for i: int in range(positions.size()):
		...
		p.y -= phase * 2
		_set_block(image,p.x,p.y,Color(SAND.r,SAND.g,SAND.b,0.72),1)
