...
static func source_kind(creature_name: String) -> String:
	...
	if ARCHETYPE_RUNTIME.has_animation(creature_name):
		return "sprite-archetype-generated"
...
static func archetype(creature_name: String) -> String:
	if SEED_ATLAS.has_name(creature_name):
		return SEED_ATLAS.archetype(creature_name)
...
static func _load_strip(creature_name: String, action: String) -> Texture2D:
	...
	if _strip_cache.has(key):
		return _strip_cache[key] as Texture2D
	if bool(_missing_cache.get(key, false)):
		return null
