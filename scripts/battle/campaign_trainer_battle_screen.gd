...
func setup_trainer(
	...
	enemy_team_index = 0
	boss_profile = BOSS_AI.info(trainer_key)
	boss_turn = 0
	boss_phase = 0
...
func _select_boss_move(moves: Array) -> Dictionary:
	if moves.is_empty():
		return {}
	var desired_pattern: String = BOSS_AI.cycle_pattern(trainer_id, boss_turn - 1, _boss_virtual_hp(), _boss_virtual_max_hp())
	var preferred: Array[String] = BOSS_AI.preferred_types(trainer_id)
	...
	for index: int in range(moves.size()):
		...
		var score: int = index
		var pattern: String = str(move.get("pattern", "direct"))
		var kind: String = str(move.get("kind", "attack"))
		var move_type: String = str(move.get("move_type", "PHYSICAL"))
		var status_id: String = str(move.get("status", ""))
		if pattern == desired_pattern:
			score += 14
		if preferred.has(move_type):
			score += 6
		if kind == "attack":
			score += 3 + int(move.get("power", 0))
			if BOSS_STATUS.damage_multiplier(move_type, player_statuses) > 1.05:
				score += 9
		if kind == "heal" and enemy_hp <= int(round(float(enemy_max_hp) * 0.45)):
			score += 14
		if kind == "guard" and enemy_hp <= int(round(float(enemy_max_hp) * 0.32)):
			score += 8
		if not status_id.is_empty() and not BOSS_STATUS.has_status(player_statuses, status_id):
			score += 4
		if desired_pattern in ["control", "setup"] and pattern in ["control", "setup", "prepared"]:
			score += 5
		if desired_pattern == "counter" and pattern in ["counter", "guard"]:
			score += 8
		if score > best_score:
			best_score = score
			best = move
...
func _update_boss_phase(lines: Array[String]) -> void:
	var new_phase: int = BOSS_AI.phase_index(trainer_id, _boss_virtual_hp(), _boss_virtual_max_hp())
	if new_phase <= boss_phase:
		return
	boss_phase = new_phase
	var text: String = BOSS_AI.phase_text(trainer_id, boss_phase)
	if not text.is_empty():
		lines.append(text)
	match BOSS_AI.mechanic(trainer_id):
		"overclock":
			BOSS_STATUS.apply(enemy_statuses, "unstable", 2)
			lines.append("AX-7 przeciąża własną osłonę: NIESTABILNY.")
		"three_phase_finale":
			if boss_phase >= 2:
				BOSS_STATUS.apply(enemy_statuses, "vulnerable", 2)
				BOSS_STATUS.apply(enemy_statuses, "unstable", 2)
				lines.append("Rdzeń Veyra otwiera się: PODATNY + NIESTABILNY.")
		"spore_regen":
			BOSS_STATUS.apply(enemy_statuses, "regen", 2)

func _apply_periodic_boss_status(lines: Array[String]) -> void:
	var periodic: Dictionary = BOSS_AI.periodic_status(trainer_id, boss_turn)
	...
	match BOSS_AI.mechanic(trainer_id):
		"stability_break":
			if boss_turn % 4 == 0 and player_stability > 0:
				player_stability = maxi(0, player_stability - 1)
				lines.append("Eron ścina rytm koncentracji: Stabilność -1.")
		"freeze_lock":
			if boss_phase >= 1 and BOSS_STATUS.has_status(player_statuses, "chilled") and boss_turn % 2 == 0:
				BOSS_STATUS.apply(player_statuses, "frozen", 1)
				lines.append("BIAŁA BLOKADA: WYCHŁODZONY przechodzi w ZAMROŻONY.")
		"prediction":
			if BOSS_STATUS.has_status(player_statuses, "marked"):
				BOSS_STATUS.apply(enemy_statuses, "focused", 1)
		"current_combo":
			if boss_phase >= 2 and BOSS_STATUS.has_status(player_statuses, "soaked"):
				BOSS_STATUS.apply(player_statuses, "paralyzed", 1)
				lines.append("SZTORM: mokre pole przewodzi impuls Veyi.")

func _apply_boss_post_turn(lines: Array[String]) -> void:
	match BOSS_AI.mechanic(trainer_id):
		"spore_regen":
			if enemy_hp > 0 and enemy_hp < enemy_max_hp:
				var heal: int = 3 + boss_phase * 2
				var before: int = enemy_hp
				enemy_hp = mini(enemy_max_hp, enemy_hp + heal)
				lines.append("Korona Aster odnawia %d HP." % (enemy_hp - before))
		"overclock":
			if boss_phase >= 1 and enemy_hp > 1 and boss_turn % 3 == 0:
				enemy_hp = maxi(1, enemy_hp - 2)
				lines.append("Przeciążenie AX-7 kosztuje 2 HP.")
		"three_phase_finale":
			if boss_phase >= 2 and enemy_trainer_stability > 0:
				enemy_trainer_stability = maxi(0, enemy_trainer_stability - 1)
				lines.append("Otwarty rdzeń traci 1 Stabilności po własnym impulsie.")
...
func _boss_virtual_max_hp() -> int:
	return maxi(1, enemy_team.size() * 100)
...
func _make_result(outcome: String, xp: int, captured_name: String = "") -> Dictionary:
	...
	if not boss_profile.is_empty():
		result["boss_mechanic"] = BOSS_AI.mechanic(trainer_id)
		result["boss_phase_reached"] = boss_phase
	if outcome == "win":
