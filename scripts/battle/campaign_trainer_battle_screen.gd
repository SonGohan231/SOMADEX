...
func _select_boss_move(moves: Array) -> Dictionary:
	...
	for index: int in range(moves.size()):
		...
		var score: int = index
		var pattern: String = str(move.get("pattern", "direct")); var kind: String = str(move.get("kind", "attack")); var move_type: String = str(move.get("move_type", "PHYSICAL")); var status_id: String = str(move.get("status", ""))
...
func _win(lines: Array[String]) -> void:
	...
	if enemy_team_index + 1 < enemy_team.size():
		...
		var enemy_base_hp: int = int(enemy_data.get("max_hp", 20)); enemy_max_hp = CAMPAIGN_BALANCE.scaled_max_hp(enemy_base_hp, enemy_level); enemy_hp = enemy_max_hp; enemy_statuses.clear(); enemy_guard = false; enemy_tex = ART.texture_for(next_name); enemy_data = _normalize_creature_moves(enemy_data); enemy_passive_id = PASSIVES.default_for_creature(enemy_data); enemy_switched_this_turn = true
...
func _make_result(outcome: String, xp: int, captured_name: String = "") -> Dictionary:
	...
	if not boss_profile.is_empty(): result["boss_mechanic"] = BOSS_AI.mechanic(trainer_id); result["boss_phase_reached"] = boss_phase
