...
func _test_full_family(names: Array, family_label: String, errors: Array[String]) -> void:
	for raw_name in names:
		...
		_expect(SPRITES.source_kind(creature_name)=="sprite-strip-authored-runtime","%s %s still routes through placeholder" % [family_label,creature_name],errors)
		for action: String in SPRITES.ACTIONS:
...
func _validate_seed_manifest(errors: Array[String]) -> void:
	...
	if file==null: return
	file.get_csv_line()
	var row_count: int=0; var families: Dictionary={}; var names: Dictionary={}
	while not file.eof_reached():
		var row: PackedStringArray=file.get_csv_line()
		if row.size()<9 or row[0].strip_edges().is_empty(): continue
		row_count+=1
		var family_id: int=int(row[0]); var creature_name: String=row[2].strip_edges()
		families[family_id]=int(families.get(family_id,0))+1
		_expect(not names.has(creature_name),"duplicate creature in seed manifest: %s" % creature_name,errors); names[creature_name]=true
