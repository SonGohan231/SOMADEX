...
func _initialize() -> void:
	...
	_expect(SPRITES.authored_full_animation_count() >= 12, "reverse pass must contain at least twelve fully animated forms", errors)
	...
	if errors.is_empty():
		print("CREATURE_SPRITE_RUNTIME_SMOKE: PASS · idle4 attack6 hurt3 faint5 special6 · families015+010+009+008 fully animated · 50-family/150-form manifest")
		quit(0)
		return
...
func _test_full_family(names: Array[String], family_label: String, errors: Array[String]) -> void:
	for creature_name: String in names:
		...
		_expect(SPRITES.source_kind(creature_name) == "sprite-strip-authored-runtime", "%s %s still routes through a placeholder source" % [family_label, creature_name], errors)
		for action: String in SPRITES.ACTIONS:
...
func _validate_seed_manifest(errors: Array[String]) -> void:
	...
	while not file.eof_reached():
		...
		if row[5].strip_edges() == "approved": approved += 1
