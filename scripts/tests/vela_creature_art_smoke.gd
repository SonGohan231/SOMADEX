...
func _validate_screen_controller(errors: Array[String]) -> void:
	...
	_expect(node.get_script() == CAMPAIGN_GAME, "Main.tscn is not using the campaign controller", errors)
	_expect(node.has_method("_open_menu"), "campaign controller lost inherited Vela art menu integration", errors)
	_expect(node.has_method("_show_next_evolution"), "campaign controller lost inherited creature art/evolution presentation", errors)
	node.free()
