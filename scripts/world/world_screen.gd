...
func setup(starter_name: String, start_tile: Vector2i, level: int, use_haptics: bool) -> void:
...
func _draw() -> void:
	...
	draw_string(font, Vector2(15,23), "VELA · MIASTECZKO", HORIZONTAL_ALIGNMENT_LEFT, 190, 12, Color("dff8f4"))
	draw_string(font, Vector2(15,43), "Rezonans lokalny: stabilny", HORIZONTAL_ALIGNMENT_LEFT, 200, 9, Color("6d9da0"))
	draw_string(font, Vector2(256,23), "TR Lv.%d" % trainer_level, HORIZONTAL_ALIGNMENT_RIGHT, 90, 11, Color("61d9d4"))
	draw_string(font, Vector2(256,43), starter, HORIZONTAL_ALIGNMENT_RIGHT, 90, 10, Color("bcd3d4"))
...
func _draw_tile(tile: Vector2i, code: String) -> void:
	var p := Vector2(tile.x * TILE, WORLD_TOP + tile.y * TILE)
	...
	match code:
		...
		"W":
			...
			var drift := int(time*10.0 + tile.y*3) % 8
			draw_line(p+Vector2(drift,7), p+Vector2(min(23,drift+9),7), Color("6fc1ca"), 1.0)
...
func _draw_player() -> void:
	var bob := 1.0 if moving and int(Time.get_ticks_msec()/100)%2 == 0 else 0.0
	var p := player_px + Vector2(3,1-bob)
...
func _draw_controls() -> void:
	...
	draw_string(font, Vector2(m.position.x+4,m.position.y+27), "MENU", HORIZONTAL_ALIGNMENT_CENTER, m.size.x-8, 11, Color("d5e9e8"))

func _draw_dialog() -> void:
	var r := Rect2(16,498,328,104)
...
func _interact() -> void:
	...
	match _tile_code(target):
		"N": dialog = "Mira: W trawie pojawiają się dzikie Somaskany.\nŁącz ruchy i obserwuj rytm przeciwnika."
		"S": dialog = "TABLICA: VELA\n↑ Stacja Somaskan     ← Staw Odbić\n→ Szlak Rezonansu"
		"C": dialog = "STACJA VELA\nSystem regeneracji jeszcze czeka na uruchomienie\nw następnym module fabuły."
	...
	if event is InputEventKey and event.pressed and not event.echo:
		if not dialog.is_empty():
			if event.keycode in [KEY_ENTER,KEY_SPACE,KEY_Z,KEY_ESCAPE,KEY_X]:
				dialog = ""
			return
		if event.keycode in [KEY_UP,KEY_W]: _request_move(Vector2i.UP)
		elif event.keycode in [KEY_DOWN,KEY_S]: _request_move(Vector2i.DOWN)
		elif event.keycode in [KEY_LEFT,KEY_A]: _request_move(Vector2i.LEFT)
		elif event.keycode in [KEY_RIGHT,KEY_D]: _request_move(Vector2i.RIGHT)
		elif event.keycode in [KEY_ENTER,KEY_SPACE,KEY_Z]: _interact()
		elif event.keycode in [KEY_ESCAPE,KEY_X,KEY_M]: menu_requested.emit(player_tile)
