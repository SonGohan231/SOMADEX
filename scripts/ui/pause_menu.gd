...
var items := ["DRUŻYNA","SOMADEX","PLECAK","TRENER","ZAPISZ","USTAWIENIA","WRÓĆ DO GRY"]
...
func _draw_section() -> void:
	var title_map := {"party":"DRUŻYNA","dex":"SOMADEX","bag":"PLECAK","trainer":"TRENER","settings":"USTAWIENIA"}
...
func _draw_party(panel: Rect2) -> void:
	...
	for i in range(moves.size()):
		...
		draw_string(font,panel.position+Vector2(24,196+i*54),"%d. %s" % [i+1,str(m["name"])],HORIZONTAL_ALIGNMENT_LEFT,270,12,Color("e1f1ef"))
		draw_string(font,panel.position+Vector2(40,214+i*54),str(m["note"]),HORIZONTAL_ALIGNMENT_LEFT,260,9,Color("76979b"))
...
func _draw_bag(panel: Rect2) -> void:
	var bag_items := [["Regenerator ×3","Leczenie Somaskana"],["Moduł chwytu ×5","Próba pozyskania dzikiego Somaskana"],["Sonda Vela ×1","Analiza pola i ukrytych sygnałów"]]
	for i in range(bag_items.size()):
		var y := 32+i*104
		draw_rect(Rect2(panel.position+Vector2(16,y),Vector2(292,84)),Color("122f38"))
		draw_string(font,panel.position+Vector2(30,y+30),bag_items[i][0],HORIZONTAL_ALIGNMENT_LEFT,250,13,Color("effaf8"))
		draw_string(font,panel.position+Vector2(30,y+53),bag_items[i][1],HORIZONTAL_ALIGNMENT_LEFT,250,9,Color("809fa2"))

func _draw_trainer(panel: Rect2) -> void:
	var paths := ["TAKTYK","OPIEKUN","BADACZ","TECHNIK","AWANGARDZISTA"]
...
func _draw_settings(panel: Rect2) -> void:
	...
	var toggle := Rect2(panel.position+Vector2(236,60),Vector2(62,32))
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		if section != "root":
			if event.keycode in [KEY_ESCAPE,KEY_X,KEY_BACKSPACE]:
				section = "root"
				queue_redraw()
			return
		if event.keycode in [KEY_UP,KEY_W]: selected = (selected + items.size()-1) % items.size()
		elif event.keycode in [KEY_DOWN,KEY_S]: selected = (selected + 1) % items.size()
		elif event.keycode in [KEY_ENTER,KEY_SPACE,KEY_Z]: _activate()
		elif event.keycode in [KEY_ESCAPE,KEY_X,KEY_M]: close_requested.emit()
		queue_redraw()
